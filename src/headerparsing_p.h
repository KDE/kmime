...
*/
[[nodiscard]] bool parseEncodedWord(const char *&scursor,
...
enum ParseTokenFlag { ParseTokenNoFlag = 0, ParseTokenAllow8Bit = 1, ParseTokenRelaxedTText = 2 };
...
[[nodiscard]] bool parseToken(const char *&scursor, const char *const send, QByteArrayView &result, ParseTokenFlags flags = ParseTokenNoFlag);
...
*/
[[nodiscard]] bool parseDotAtom(const char *&scursor, const char *const send, QByteArrayView &result, bool isCRLF = false);

/** @p scursor must be positioned after the opening openChar. */
[[nodiscard]] bool
parseGenericQuotedString(const char *&scursor, const char *const send, QString &result, bool isCRLF, const char openChar = '"', const char closeChar = '"');

/** @p scursor must be positioned right after the opening '(' */
[[nodiscard]] bool parseComment(const char *&scursor, const char *const send, QString &result, bool isCRLF = false, bool reallySave = true);
...
/**
  ...
  @return true if the input phrase was successfully parsed; false otherwise.
*/
[[nodiscard]] bool parsePhrase(const char *&scursor, const char *const send, QString &result, bool isCRLF = false);
 ...
 */
[[nodiscard]] bool
parseParameterListWithCharset(const char *&scursor, const char *const send, KMime::Headers::ParameterMap &result, QByteArray &charset, bool isCRLF = false);

[[nodiscard]] bool parseDomain(const char *&scursor, const char *const send, QString &result, bool isCRLF = false);

[[nodiscard]] bool parseObsRoute(const char *&scursor, const char *const send, QStringList &result, bool isCRLF = false, bool save = false);

[[nodiscard]] bool parseAddrSpec(const char *&scursor, const char *const send, Types::AddrSpec &result, bool isCRLF = false);

[[nodiscard]] bool parseAngleAddr(const char *&scursor, const char *const send, Types::AddrSpec &result, bool isCRLF = false);
...
[[nodiscard]] bool
parseTime(const char *&scursor, const char *const send, int &hour, int &min, int &sec, long int &secsEastOfGMT, bool &timeZoneKnown, bool isCRLF = false);

[[nodiscard]] bool parseDateTime(const char *&scursor, const char *const send, QDateTime &result, bool isCRLF = false);
[[nodiscard]] bool parseQDateTime(const char *&scursor, const char *const send, QDateTime &result, bool isCRLF = false);
}

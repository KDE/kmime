...
static QByteArray finalRecipient(const QString &recipient)
{
    if (recipient.isEmpty()) {
        ...
        return "Final-Recipient: rfc822; " + encodeRFC2047String(recipient, "utf-8") + '\n';
    }
}
...
static QByteArray reportingUAField()
{
    ...
    return QByteArray("Reporting-UA: ") + QByteArray(hostName) + QByteArray("; KMime " KMIME_VERSION_STRING "\n");
}
...
QString descriptionFor(DispositionType d, const QList<DispositionModifier> &)
{
    ...
    qCWarning(KMIME_LOG) << "KMime::MDN::descriptionFor(): No such disposition type:" << static_cast<int>(d);
    return {};
}
